...
pickles the result -- exactly like :mod:`data.build_network`, but bounded by a
box rather than a centre+radius so the network lines up with the chosen zones.

The cached graph is consumed by :class:`~ridepool_sim.osmnx_network.OSMnxNetwork`
(``graph_path=...``), the same backend used for the Guomao network, so the rest
of the simulation is unchanged.
...
DEFAULT_OUT = os.path.join(_HERE, "manhattan.gpickle")
...
def build_nyc_network(
    ...
    out_path:
        ...
        Guomao network so :class:`OSMnxNetwork` loads it unchanged).
    """
